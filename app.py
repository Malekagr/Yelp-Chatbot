...
#the bot has now given options for the 
#handles the invokers jobs, finalizing the vote, rerolling, cancelling vote  
def invoker_jobs(event_data):
  message = event_data["event"]
    if message.get("subtype") is None and not message.get("text") is None:
        text = message["text"]
        channel = message["channel"]
